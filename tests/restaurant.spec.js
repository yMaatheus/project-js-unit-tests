...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('O retorno da função createMenu() é um objeto que possui a chave fetchMenu, a qual tem como valor uma função.', () => {
    ...
    // ```
    const objetoRetornado = createMenu(); // { fetchMenu: () => {}, ... }
    expect(validateTypeofAndKey(objetoRetornado)).toBe(true);
  })
  it('ObjetoRetornado.fetchMenu() retorna um objeto cujas chaves são somente `food` e `drink`, considerando que a função createMenu() foi chamada com o objeto: `{ food: {}, drink: {} }`.', () => {
  ...
  it('O menu passado pra função createMenu é identico ao menu recuperado pela função objetoRetornado.fetchMenu', () => {
    ...
    const objetoRetornado = createMenu(objetoQualquer);
    expect(objetoQualquer).toEqual(objetoRetornado.fetchMenu());
  ...
  it('Ao chamar uma função associada à chave `order` no objeto retornado, passando uma string como parâmetro (como `objetoRetornado.order(`coxinha`)`), tal string é adicionada ao array retornado em `objetoRetornado.consumption`.', () => {
  ...
  it('Ao chamar `objetoRetornado.pay()`, retorna-se a soma dos preços de tudo que foi pedido, conforme registrado em `objetoRetornado.consumption`', () => {
    ...
    objetoRetornado.order('agua');
    objetoRetornado.order('coxinha');
    expect(objetoRetornado.pay()).toBe(11.70);
    // Agora faça o PASSO 4 no arquivo `src/restaurant.js`.
  })
});

function validateTypeofAndKey(result) {
  if (typeof result === 'object' && Object.keys(result).includes('fetchMenu')) {
    return true;
  }
  return false;
}